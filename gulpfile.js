...
gulp.task('release', ['bake:release', 'copy'], function (cb)
{
	...
	var css = fs.readFileSync('build/sunblast.min.css', 'utf-8');
	var js = fs.readFileSync('build/sunblast.min.js', 'utf-8');

	formatJson.source = formatJson.source.replace(/<link .*?sunblast\.min\.css.*?>/i, '<style>' + css + '</style>');
	formatJson.source = formatJson.source.replace(/<script .*?sunblast\.min\.js.*?>/i, '<script>' + js + '</script>');
